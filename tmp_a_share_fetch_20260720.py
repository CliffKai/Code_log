...
SERIES=[('上证指数','000001.SH','1.000001','东方财富指数日K，不复权'),('科创50','000688.SH','1.000688','东方财富指数日K，不复权'),('创业板指','399006.SZ','0.399006','东方财富指数日K，不复权'),('CPO概念','BK1128','90.BK1128','东方财富概念板块日K，不复权'),('PCB','BK0877','90.BK0877','东方财富概念板块日K，不复权'),('存储芯片','BK1137','90.BK1137','东方财富概念板块日K，不复权'),('半导体设备','BK1326','90.BK1326','东方财富概念板块日K，不复权'),('半导体材料','BK1325','90.BK1325','东方财富概念板块日K，不复权')]
...
HEADERS={'User-Agent':'Mozilla/5.0 Chrome/126 Safari/537.36','Referer':'https://quote.eastmoney.com/','Accept':'application/json,text/plain,*/*'}
def fetch(secid):
 p=urllib.parse.urlencode({'secid':secid,'klt':'101','fqt':'0','fields1':'f1,f2,f3,f4,f5,f6','fields2':'f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61','lmt':'100000','beg':BEG.replace('-',''),'end':TARGET.replace('-','')})
 errs=[]
 for host in ['push2his.eastmoney.com']+[f'{i}.push2his.eastmoney.com' for i in range(1,100)]:
  url=f'https://{host}/api/qt/stock/kline/get?{p}'
  for attempt in range(3):
   try:
    with urllib.request.urlopen(urllib.request.Request(url,headers=HEADERS),timeout=30) as r: payload=json.loads(r.read().decode())
    if payload.get('rc')==0 and payload.get('data',{}).get('klines'): return payload,url
   except Exception as e: errs.append(f'{host} {type(e).__name__}: {e}'); time.sleep(1+attempt)
 raise RuntimeError('all hosts failed: '+' | '.join(errs[-10:]))
def parse(payload):
 rows=[]
...
fetched_at=datetime.now(timezone(timedelta(hours=8))).isoformat(timespec='seconds'); results=[]; validations=[]; calendars={}
for name,code,secid,scope in SERIES:
 payload,url=fetch(secid); (RAW/f"{secid.replace('.','_')}.json").write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding='utf-8')
 rows=parse(payload); by={r['date']:r for r in rows}; cur=by.get(TARGET); prev=[r for r in rows if r['date']<TARGET][-1]; yend=[r for r in rows if r['date']<'2026-01-01'][-1]
 if cur is None: raise RuntimeError(f'{name} missing target')
 rec_chg=cur['close']-prev['close']; rec_pct=(cur['close']/prev['close']-1)*100; diff_bp=(cur['supplier_pct']-rec_pct)*100; ytd=(cur['close']/yend['close']-1)*100
 ohlc=cur['low']<=min(cur['open'],cur['close']) and cur['high']>=max(cur['open'],cur['close']) and cur['low']<=cur['high']; calendars[name]={r['date'] for r in rows if r['date']>='2026-01-01'}
...
with (OUT/'target_day_20260720.csv').open('w',encoding='utf-8-sig',newline='') as f: w=csv.DictWriter(f,fieldnames=cols); w.writeheader(); w.writerows(results)
(OUT/'validation.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8'); (OUT/'results.json').write_text(json.dumps(results,ensure_ascii=False,indent=2),encoding='utf-8')
