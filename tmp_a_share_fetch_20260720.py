...
def fetch(secid):
    params=urllib.parse.urlencode({'secid':secid,'klt':'101','fqt':'0','fields1':'f1,f2,f3,f4,f5,f6','fields2':'f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61','lmt':'100000','beg':BEG.replace('-',''),'end':TARGET.replace('-','')})
    hosts=['push2his.eastmoney.com']+[f'{i}.push2his.eastmoney.com' for i in range(1,100)]
    errs=[]
    for host in hosts:
        url=f'https://{host}/api/qt/stock/kline/get?{params}'
...
for name,code,secid,scope in SERIES:
    ...
    prev=[r for r in rows if r['date']<TARGET][-1]
    yend=[r for r in rows if r['date']<'2026-01-01'][-1]
